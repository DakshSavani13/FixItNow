...
const StaffDashboard = () => {
  ...
  // Cache of staff options per category for category-wise assignment
  ...
  // Get maintenance staff filtered by category with local cache
  const getStaffForCategory = async (category) => {
    ...
    try {
      ...
      // Fallback to all maintenance staff if none match the category
      const finalList = list.length > 0 ? list : maintenanceStaff;
      setCategoryStaffMap(prev => ({ ...prev, [category]: finalList }));
      return finalList;
    } catch (e) {
      // On error, at least provide full maintenance list as a fallback
  ...
  const handleAddStaff = async (e) => {
    ...
    try {
      // Generate a temporary strong password since the API requires it
      const tempPassword = `Temp@${Math.random().toString(36).slice(2, 8)}${Date.now().toString().slice(-2)}`;
      ...
      const staffData = {
        ...
        role: 'maintenance',
        categories: addStaffForm.categories
      };
      
      await axios.post('/admin/maintenance-staff', staffData);
      setShowAddStaffModal(false);
      setAddStaffForm({ name: '', email: '', phone: '', department: '', categories: [] });
      fetchStaff();
    } catch (err) {
  ...
  const handleDeleteStaff = async (id) => {
    ...
    setStaffActionLoading(id + '-delete');
    try {
                    ...
                    <div className="complaint-actions">
                      <div className="action-group">
                        ...
                        <CategoryAssignSelect
                          ...
                          onChange={(id) => handleAssignComplaint(complaint._id, id)}
                          loading={complaintActionLoading === complaint._id + '-assign'}
                  ...
                  <div key={member._id} className="staff-card">
                    ...
                    <div className="staff-actions">
                      <button
                        ...
                        disabled={staffActionLoading === member._id + '-delete'}
                      >
                        <FaTimes /> Delete
                      </button>
                    </div>
          ...
          <div className="modal-card">
            ...
            <div className="modal-body">
              <form onSubmit={handleAddStaff}>
                ...
                <div className="form-group">
                  <label>Complaints Categories (skills)</label>
                  <select
                    multiple
                    value={addStaffForm.categories}
                    onChange={e => setAddStaffForm({ ...addStaffForm, categories: Array.from(e.target.selectedOptions, o => o.value) })}
                    className="form-control"
                  >
                    {['electrical','plumbing','furniture','wifi','heating','cleaning','other'].map(opt => (
...
// Helper sub-component to render category-filtered staff select for assignment
