...
import { FaUsers, FaClipboardList, FaTools, FaChartBar, FaDownload, FaUserCog, FaListAlt, FaCheck, FaTimes, FaUserShield, FaUser, FaPlus, FaEdit, FaTrash } from 'react-icons/fa';
...
const AdminDashboard = () => {
  ...
  const [addForm, setAddForm] = useState({ name: '', email: '', password: '', phone: '', role: 'maintenance', department: '', categories: [] });
  ...
  const handleAddMaintenance = async (e) => {
    ...
    try {
      ...
      setAddForm({ name: '', email: '', password: '', phone: '', role: 'maintenance', department: '', categories: [] });
      fetchUsers(userPage);
    } catch (err) {
                      ...
                      <tr key={user._id}>
                        ...
                        <td>
                          <button
                            ...
                            disabled={userActionLoading === user._id + '-delete'}
                          >
                            <FaTrash /> Delete
                          </button>
                        </td>
              ...
              <div className="modal-overlay">
                <div className="modal-card">
                  ...
                  <form onSubmit={handleAddMaintenance}>
                    ...
                    {(addForm.role === 'staff' || addForm.role === 'maintenance') && (
                      <div className="form-group">
                        <label>Department {addForm.role === 'staff' ? '*' : ''}</label>
                        <input
                          type="text"
                          value={addForm.department}
                          onChange={e => setAddForm({ ...addForm, department: e.target.value })}
                          className="form-control"
                          placeholder="e.g., IT, Administration, Electrical"
                          required={addForm.role === 'staff'}
                        />
                      </div>
                    )}
                    {addForm.role === 'maintenance' && (
                      <div className="form-group">
                        <label>Skills/Categories</label>
                        <div className="checkbox-group">
                          {['electrical', 'plumbing', 'furniture', 'wifi', 'heating', 'cleaning', 'other'].map(cat => (
                            <label key={cat} className="checkbox-label">
                              <input
                                ...
                                onChange={e => {
                                  if (e.target.checked) {
                                    setAddForm({ ...addForm, categories: [...addForm.categories, cat] });
                                  } else {
                                    setAddForm({ ...addForm, categories: addForm.categories.filter(c => c !== cat) });
                                  }
                                }}
                              />
