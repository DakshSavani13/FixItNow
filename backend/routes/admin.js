...
// Require authentication for all admin routes; authorization is applied per-route below
router.use(auth);
...
// Get all users
router.get('/users', authorize('admin', 'staff'), async (req, res) => {
  try {
...
// Update user details (categories/phone/department/name/email). Role update allowed only for admin/staff.
router.patch('/users/:id', authorize('admin', 'staff', 'maintenance'), async (req, res) => {
  try {
    const { name, email, phone, department, categories, role } = req.body;
    ...
    // Only admin/staff can change role
    if (typeof role === 'string' && (req.user.role === 'admin' || req.user.role === 'staff')) {
      update.role = role;
    }
...
router.get('/maintenance-staff', authorize('admin', 'staff', 'maintenance'), async (req, res) => {
  try {
    const { category } = req.query;
    const filter = { role: 'maintenance', isActive: true };
    if (category) {
...
router.post('/maintenance-staff', authorize('admin', 'staff', 'maintenance'), async (req, res) => {
  try {
    const { name, email, password, phone, department, role = 'maintenance', categories = [] } = req.body;
    ...
    // Attach categories (skills) for maintenance
    if (role === 'maintenance' && Array.isArray(categories)) {
...
// Update user status
router.patch('/users/:id/status', authorize('admin', 'staff'), async (req, res) => {
  try {
...
// Delete user
router.delete('/users/:id', authorize('admin', 'staff'), async (req, res) => {
  try {
...
// Get analytics data
router.get('/analytics', authorize('admin', 'staff'), async (req, res) => {
  try {
...
router.get('/export', authorize('admin', 'staff'), async (req, res) => {
  try {
